...
EDA is generally used to see whether there are correlation between variables and Salary. Boxplot and ANOVA would be working to see if factors are highly correlated for categorical variables. Heatmap and Vif function would be working to see if factors are highly correlated for numerical variables. All factors which are correlated to salary will be counted as a factor in the model. Also, whether there is correlation between these variables are also analyzed to see if there are interactive effects for those variables. \## Multilevel Linear Model Two models are being fitted, one contains the interactive effect and the other doesn't. For both models, the assumptions of the multilevel model are checked whether the model fits the requirement. For each multilevel model, the linearity, normality assumption, and homogeneity of variance are being checked using the model. \# Result \## Visualization \### cateforical variables

```{r}
#| warning: false
#| message: false
#| echo: false
# Boxplot for different countries
